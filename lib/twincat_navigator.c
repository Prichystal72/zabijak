...
HWND FindProjectListBox(HWND parentWindow) {
    HWND leftmostListBox = NULL;
    int leftmostPosition = 99999;
    RECT parentRect;
    ...
    while (childWindow != NULL) {
        ...
        if (strcmp(className, "ListBox") == 0) {
            ...
            int relativeLeft = rect.left - parentRect.left;

            if (relativeLeft < leftmostPosition) {
                leftmostPosition = relativeLeft;
                leftmostListBox = childWindow;
            }
        }
...
//   "TwinCAT PLC Control - [Aktion Robot01_PosPal - Exit (ST) - SpecialMode06 (PRG-SFC)]" -> "SpecialMode06"
bool ExtractTargetFromTitle(const char* windowTitle, char* targetText, int maxLength) {
    if (!windowTitle || !targetText || maxLength < 1) {
        printf("[DEBUG] ExtractTargetFromTitle: Neplatné parametry\n");
        return false;
    }
    
    targetText[0] = '\0';
    
    printf("[DEBUG] Parsing titulku: '%s'\n", windowTitle);
    ...
    if (!start || !end || end <= start) {
        printf("[DEBUG] Nenalezeny závorky []\n");
        return false;
    }
    ...
    printf("[DEBUG] Text mezi závorkami: '%s'\n", buffer);
    
    // DETEKCE SFC: "NĚCO (TYP) - PROGRAM (PRG-SFC)" nebo "PROGRAM (PRG-SFC)"
    if (strstr(buffer, "(PRG-SFC)")) {
        printf("[DEBUG] Detekován SFC pattern!\n");
        
        // Najdi "(PRG-SFC)"
        char* prg_sfc = strstr(buffer, "(PRG-SFC)");
        if (prg_sfc) {
            // Kontrola, jestli obsahuje " - " (akce v SFC)
            char* last_dash = NULL;
            char* search_pos = prg_sfc;
            
            // Hledej poslední " - " před "(PRG-SFC)"
            while (search_pos > buffer) {
                search_pos--;
                if (search_pos > buffer + 2 && strncmp(search_pos, " - ", 3) == 0) {
                    last_dash = search_pos;
                    break;
                }
            }
            
            if (last_dash) {
                // PŘÍPAD 1: "ACTION - PROGRAM (PRG-SFC)"
                printf("[DEBUG] SFC s akcí/krokem\n");
                last_dash += 3; // Přeskoč " - "
                
                // Extrahuj název programu
                char program_name[256];
                int program_len = (int)(prg_sfc - last_dash);
                if (program_len > 0 && program_len < sizeof(program_name)) {
                    strncpy(program_name, last_dash, program_len);
                    program_name[program_len] = '\0';
                    
                    // Odstraň mezery na konci
                    while (program_len > 0 && program_name[program_len-1] == ' ') {
                        program_name[--program_len] = '\0';
                    }
                    
                    printf("[DEBUG] SFC Program: '%s'\n", program_name);
                    
                    // Extrahuj název akce/kroku (před " - ")
                    char action_name[256];
                    int action_len = (int)(last_dash - 3 - buffer);
                    if (action_len > 0 && action_len < sizeof(action_name)) {
                        strncpy(action_name, buffer, action_len);
                        action_name[action_len] = '\0';
                        printf("[DEBUG] SFC Action/Step: '%s'\n", action_name);
                    }
                    
                    // Vrátíme název programu
                    strncpy(targetText, program_name, maxLength - 1);
                    targetText[maxLength - 1] = '\0';
                    
                    printf("[DEBUG] Výsledek (SFC program): '%s'\n", targetText);
                    return true;
                }
            } else {
                // PŘÍPAD 2: "PROGRAM (PRG-SFC)" - jen samotný program
                printf("[DEBUG] SFC bez akce - jen program\n");
                
                // Extrahuj název programu (celý buffer bez " (PRG-SFC)")
                char program_name[256];
                int program_len = (int)(prg_sfc - buffer);
                if (program_len > 0 && program_len < sizeof(program_name)) {
                    strncpy(program_name, buffer, program_len);
                    program_name[program_len] = '\0';
                    ...
                    printf("[DEBUG] SFC Program (samotný): '%s'\n", program_name);
        ...
        printf("[DEBUG] Chyba při parsování SFC pattern\n");
        return false;
    }
    
    printf("[DEBUG] Standardní parsing (není SFC Action)\n");
    
    // STANDARDNÍ PARSING pro obyčejné soubory
    ...
    printf("[DEBUG] Název souboru: '%s'\n", fileName);
    
    // Odstraň příponu (.pro, .EXP, atd.)
    char* dot = strchr(fileName, '.');
    if (dot) {
        *dot = '\0';
        printf("[DEBUG] Po odstranění přípony: '%s'\n", fileName);
    }
    ...
    if (dash) {
        *dash = '\0';
        printf("[DEBUG] Po odstranění suffix: '%s'\n", fileName);
    }
    ...
    if (bracket) {
        ...
        *bracket = '\0';
        printf("[DEBUG] Po odstranění závorek: '%s'\n", fileName);
    }
    ...
    printf("[DEBUG] Finální výsledek: '%s'\n", targetText);
